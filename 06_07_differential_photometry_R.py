        ...
        try:
            ...
            for _, row  in df_light.iterrows():
                ...
                if SOLVE :
                    ...
                    axs[0].plot(phot_stars_na["Rmag"], phot_stars_na["mag"] - phot_stars_na["Rmag"], '+')
                    axs[1].plot(phot_stars_na["grcolor"], phot_stars_na["mag"] - phot_stars_na["Rmag"], '+')
                    _xx = np.linspace(10, 15)
                    axs[2].plot(phot_stars_na["Rmag"], phot_stars_na["mag"], '+')
                    y0 = np.median(phot_stars_na["mag"] - phot_stars_na["Rmag"])
                    axs[2].plot(_xx, _xx + y0,
                            label=f"R_inst = R magnitude + ({y0:.01f})"
                            )
                    ...
                    for _, row in phot_stars_na.iterrows():
                        ...
                        axs[2].text(row["Rmag"], row["mag"], int(row["id"]), fontsize=8)
